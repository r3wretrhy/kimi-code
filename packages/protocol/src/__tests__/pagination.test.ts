...
describe('pagination — CursorQuery', () => {
  ...
  it('rejects page_size above 100 (SCHEMAS.md §1.3 hard upper bound)', () => {
    // SCHEMAS.md §1.3 specifies the range as `1..100`. Behavior on overflow is
    // rejection (no silent clamping) — endpoints should surface validation.failed
    // back to the client so callers learn the cap.
